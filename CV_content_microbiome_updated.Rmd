...
setwd("/Users/michelle/Documents/German Lab Research Mac/All_Cv_Microbiome/R_stuff")
...
#unzip(zipfile = "Cv-content-CA-filtered-table_silva.qza")

#Only need to unzip once. Rename the folder in your computer
#unzip(zipfile = "taxonomy_silva.qza")

#Check for these files in the 'files' tab. Rename them unzip_content_table, unzip_intestine_table, unzip_taxonomy
...
my_biom <- read_hdf5_biom("./unzip_content_table/data/feature-content-table.biom") #Change here within quotes.
...
write.table(OTU_table, file = "OTU_table_CV_content_only.tsv", quote=FALSE, sep='\t', col.names = NA)
...
write.table(t(rared_OTU), file = "OTU_filtered_plus_rarefied_content_only.tsv", quote=FALSE, sep='\t', col.names = NA)
...
ggplot(data = merged_alpha) +
  ...
  labs(title = 'Shannon alpha-diversity', subtitle = bquote('P = 0.046,'~F~ '= 3.52'), 
...
ggsave(filename="Alpha_diversity_CV_contents.png") 
...
aes(x = MDS1, y = MDS2, color = Diet)+ #Creates and colors legend to match, modify after $.
  ...
  ggtitle("CV content only NMDS plot", subtitle = bquote('P < 0.001,'~R^2~ '= 0.396')) + #Adds tittle and subtitle. 
...
ggplot(data = nmds_plus_metadata) +
  ...
  geom_point(aes(shape=Diet, color=Diet), size=7) +
  scale_shape_manual(values=c(19, 19, 19, 19))+
  scale_color_manual(values=c('tomato','steelblue', 'plum3', 'gray50'))+ 
  labs(col = "Diet") + #Renames legend, modifiable within quotes.
  ggtitle("NMDS FHL wild contents only", subtitle = bquote('P < 0.001,'~R^2~ '= 0.396')) +#Adds tittle and subtitle. 
  # Can modify p and r-squared values + title.
  theme_classic(base_size = 14, base_line_size = .5)

#save nmds plot
ggsave(filename="NMDS_CV_content_nmds_shapes_colors_no_chloro_mito.png")
```
...
```{r add vectors to plot, choosing cutoff}
#try estimate as a filter has 34 taxa
correlations %>%
...
ggplot(data = nmds_plus_metadata) +
  aes(x = MDS1, y = MDS2)+ 
  geom_point(aes(shape=Diet, color=Diet), size=7, fill = 'tomato') + 
  ...
  scale_shape_manual(values=c(25, 18, 17, 16)) +
  scale_color_manual(values=c('tomato','steelblue', 'plum3', 'gray50'))+ 
  labs(col = "Diet") + 
  ggtitle("NMDS CV content", subtitle = bquote('P < 0.001,'~R^2~ '= 0.396')) + 
  # Can modify p and r-squared values + title.
  theme_classic(base_size = 14, base_line_size = .5)

ggsave(filename="NMDS_vectors_names_Cv_content_no_chloro_mito.png") 

#get rid of vector labels
ggplot(data = nmds_plus_metadata) +
  aes(x = MDS1, y = MDS2)+ 
  geom_point(aes(shape=Diet, color=Diet), size=6, fill = 'tomato') + 
  geom_segment(data=high_corr, aes(x=0, xend=estimate.x, y=0, yend=estimate.y)) +
  scale_shape_manual(values=c(25, 18, 17, 16)) +
  scale_color_manual(values=c('tomato','steelblue', 'plum3', 'gray50'))+ 
  labs(col = "Diet") + 
...
ggsave(filename="NMDS_vectors_no_names_Cv_content_no_chloro_mito.png", width=7, height=5) 
