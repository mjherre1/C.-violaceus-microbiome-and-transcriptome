...
#unzip(zipfile = "Cv-intestine-CA-filtered-table.qza")

#Only need to unzip once. Rename the folder in your computer
#unzip(zipfile = "taxonomy_silva.qza")

#Check for these files in the 'files' tab. Rename them unzip_intestine_table, unzip_intestine_table, unzip_taxonomy
...
my_biom <- read_hdf5_biom("./unzip_intestine_table/data/feature-intestine-table.biom")
...
write.table(OTU_table, file = "OTU_table_Cv_intestine.tsv", quote=FALSE, sep='\t', col.names = NA)
...
write.table(t(rared_OTU), file = "OTU_filtered_plus_rarefied_intestine_only.tsv", quote=FALSE, sep='\t', col.names = NA)
...
ggplot(data = merged_alpha) +
  ...
  labs(title = 'Shannon alpha-diversity', subtitle = bquote('P = 0.149,'~F~ '= 2.11'), 
...
ggsave(filename="Alpha_diversity_CV_intestines.png") 
...
aes(x = MDS1, y = MDS2, color = Diet)+ #Creates and colors legend to match, modify after $.
  ...
  ggtitle("CV intestine only NMDS plot", subtitle = bquote('P < 0.001,'~R^2~ '= 0.457, Stress = 0.109')) + #Adds tittle and subtitle. 
...
ggplot(data = nmds_plus_metadata) +
  ...
  labs(col = "Diet") + #Renames legend, modifiable within quotes.
  ggtitle("NMDS CV intestines only", subtitle = bquote('P < 0.001,'~R^2~ '= 0.457, Stress = 0.109')) +#Adds tittle and subtitle. 
  # Can modify p and r-squared values + title.
  theme_classic(base_size = 14, base_line_size = .5)

#save nmds plot
ggsave(filename="NMDS_CV_intestine_nmds_shapes_colors_no_chloro_mito.png", width=10, height=7)
```
...
```{r add vectors to plot, choosing cutoff}
#try estimate as a filter has 89 taxa
correlations %>%
...
ggplot(data = nmds_plus_metadata) +
  ...
  scale_color_manual(values=c('tomato','steelblue', 'plum3', 'gray50'))+ 
  labs(col = "Diet") + 
  ggtitle("NMDS CV intestine", subtitle = bquote('P < 0.001,'~R^2~ '= 0.457, Stress = 0.109')) + 
  # Can modify p and r-squared values + title.
  theme_classic(base_size = 14, base_line_size = .5)

ggsave(filename="NMDS_vectors_names_Cv_intestine_no_chloro_mito.png") 
...
ggsave(filename="NMDS_vectors_no_names_Cv_intestine_no_chloro_mito.png", width=7, height=5) 
